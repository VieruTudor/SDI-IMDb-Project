...
import controllers.FutureActorController;
import controllers.FutureDirectorController;
import controllers.FutureMovieController;
import controllers.FuturePlaysInController;
...
public class Console {
    ...
    @Autowired
    private FutureActorController autowiredActorController;
    @Autowired
    private FutureMovieController autowiredMovieController;
    @Autowired
    private FutureDirectorController autowiredDirectorController;
    @Autowired
    private FuturePlaysInController autowiredPlaysInController;

    @PostConstruct
    private void initialize() {
        Console.actorController = autowiredActorController;
        Console.movieController = autowiredMovieController;
        Console.directorController = autowiredDirectorController;
        Console.playsInController = autowiredPlaysInController;
        try {
    ...
    private void filterActor() {
        ...
        try {
            ...
            CompletableFuture.supplyAsync(
                    () -> actorController.getActorsWithFameBetween(low, high)
            )
    ...
    private void reportActors() {
        try {
            var fame = Integer.parseInt(this.getField("fame"));
            CompletableFuture.supplyAsync(
                    () -> actorController.getPercentageOfFamousActors(fame)
            )
                    .thenAcceptAsync(System.out::println);
